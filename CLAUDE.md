...
- **Import history is one-way**: copies `~/.claude/projects/<encoded-cwd>/` (path separators → dashes, see `encodeCwdToProjectFolder`) into the profile's session dir. Never reads-then-writes back to `~/.claude/`.
